...
import { getTalkToUsEntries } from "@/lib/actions"
...
export default function TalkToUsAdmin() {
  const [entries, setEntries] = useState<TalkToUsEntry[]>([])
  const [loading, setLoading] = useState(true)
  const [searchTerm, setSearchTerm] = useState("")
  const [sortField, setSortField] = useState<"createdAt" | "name">("createdAt")
  const [sortOrder, setSortOrder] = useState<"asc" | "desc">("desc")
  const [selectedEntry, setSelectedEntry] = useState<TalkToUsEntry | null>(null)
  ...
  const loadEntries = async () => {
    try {
      const data = await getTalkToUsEntries()
      setEntries(data as TalkToUsEntry[])
    } catch (error) {
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-gray-600 dark:text-gray-400 mt-2">
              Manage and review consultation requests and general inquiries.
            </p>
          </div>
